...
# 2B. 在 Codex 中使用
# 直接打开本仓库并描述任务。Codex 会读取 AGENTS.md，
# 按同一套 prompts、gates、references 和 templates 执行。
...
| Agent 支持 | Claude Code Skill；Codex 通过 `AGENTS.md` 使用同一工作流 |
...
本仓库同时支持 Claude Code 和 Codex。Claude Code 的入口是
[`.claude/skills/scholar-reach/SKILL.md`](.claude/skills/scholar-reach/SKILL.md)，
Codex 的入口是 [`AGENTS.md`](AGENTS.md)。

两个入口使用同一套阶段、Human Gates、claim ledger、渠道规范和输出模板。使用 Codex 时，可以直接提出任务，例如“请为当前论文生成中英文传播包”或“请读取某个 GitHub 仓库并生成 X 与小红书草案”。Codex 会先读取对应的 `prompts/`、`gates/`、`references/` 和 `templates/`，再生成或修改内容。

Codex 仍遵守本项目的核心边界：不发布内容，不强化证据之外的结论，不编造指标，不在生成传播包时修改用户原始论文、数据或目标项目 README。Git commit、push、release 等仓库维护操作只在用户明确要求时执行。
...
在 Codex 中使用时，打开本仓库并直接描述任务即可。Codex 会读取
[`AGENTS.md`](AGENTS.md)，并按同一套工作流执行。
